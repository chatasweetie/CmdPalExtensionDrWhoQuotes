...
$csprojPath = Join-Path $projectRoot "CmdPalExtensionDrWhoQuotes.csproj"
...
if ($backupDir) {
    ...
    try {
        Copy-Item $csprojPath -Destination (Join-Path $backupDir "CmdPalExtensionDrWhoQuotes.csproj.$timestamp.bak") -Force
...
# Update .csproj file
Write-Host "[2/2] Updating CmdPalExtensionDrWhoQuotes.csproj..." -ForegroundColor Cyan
...
if ($csprojContent -ne $csprojBackup) {
    try {
        ...
        Write-Host ""
        Write-Host "  [SUCCESS] CmdPalExtensionDrWhoQuotes.csproj updated ($csprojUpdateCount changes)" -ForegroundColor Green
    }
    catch {
        Write-Host ""
    ...
    Write-Host ""
    Write-Host "  [INFO] No changes were needed for CmdPalExtensionDrWhoQuotes.csproj" -ForegroundColor Cyan
}
Write-Host ""
